...
function FW_PH( bnb_node::node, V_0::Array{AbstractArray{Vector{Array{Float64}},1}}, x_0::Array{Array{Float64}}, w_0::Array{Array{Float64}}, number_of_nodes_used::Int )
    ...
    #for k = 1:1
        ...
        if (sqrt(sum( initial_parameters.scen_prob[s] .* norm(x_k[k,s] .- (k == 1 ? z_0 : z_k[k-1]) )^2 for s = 1 : initial_parameters.num_scen)) < PH_ϵ) || (time()-FW_start_time) > initial_parameters.PH_SDM_parameters.PH_max_time
            ...
            if initial_parameters.PH_SDM_parameters.PH_plot && (number_of_nodes_used == 1)
                return (x_k[k, :], y_k[k, :], w_RNMDT_k[k, :], z_FR_k[k,:], z_k[k], w_k[k, :], ϕ_k[1:k], dual_feasibility_condition, primal_dual_residial, feasibility_set, identical_appearance_count, k)
            else
                return (x_k[k, :], y_k[k, :], w_RNMDT_k[k, :], z_FR_k[k,:], z_k[k], w_k[k, :], ϕ_k[1:k], [], [], [], [], k)
            end
        end
    ...
    if initial_parameters.PH_SDM_parameters.PH_plot && (number_of_nodes_used == 1)
        return (x_k[end, :], y_k[end, :], w_RNMDT_k[end, :], z_FR_k[end,:], z_k[end], w_k[end, :], ϕ_k[1:end], dual_feasibility_condition, primal_dual_residial, feasibility_set, identical_appearance_count, k_max)
    else
        return (x_k[end, :], y_k[end, :], w_RNMDT_k[end, :], z_FR_k[end,:], z_k[end], w_k[end, :], ϕ_k[1:end], [], [], [], [], k_max)
    end
