...
function bnb_solve(initial_parameters::MIP_initial_parameters, non_ant_tol::Float64, tol_bb::Float64, integrality_tolerance::Float64)
    ...
    if initial_parameters.pool_problem_is_used 
        initial_parameters = zero_node_generation(initial_parameters)[2]
    end
    ...
    #while i <= 1
        ...
        if initial_parameters.al_is_used
            ...
            # if the Augmented Lagrangian relaxation penalty parameter is allowed to be updated (improved)
            if !initial_parameters.al_is_fixed && (number_of_nodes_used > 1)
                # generate new AL penalty paramter vector
        ...
        # calcuate the delta
        current_delta = round.(delta_computation(current_node_output.variables_values[1]), digits = 6)
        #@show current_delta
