...
function experiments_function(n_scenarios, fs_var, ss_var, const_num, experiments_methods, g_time_limit, p_values, output_link, AL, r_seed )
    ...
    if sum(experiments_methods) == length(experiments_methods)
        ...
        output_df = DataFrame( num_of_scen = Int[], num_fs_var = Int[], num_ss_var = Int[], num_const = Int[], p_RNMDT = Int[], primal_f = Float64[], primal_x = String[], primal_gap = Float64[], RNMDT_UB = Float64[], RNMDT_x = String[], RNMDT_time = Float64[], RNMDT_wy_gap = Float64[], Solution_method = String[], BnB_UB = Float64[], BnB_LB = Float64[], BnB_x = String[], BnB_time = Float64[], BnB_wy_gap = Float64[], BnB_nodes_explored = Int[] )
            ...
            for i_fs_var in fs_var
                for i_ss_var in ss_var
                    for i_const_num in const_num
                        ...
                        initial_parameters = initialisation(s,i_fs_var,i_ss_var,i_const_num, p, AL, r_seed)
                        ...
                        while (bnb_g_time < g_time_limit) && (p_index <= length(p_values))
                            ...
                            if initial_parameters.PH_SDM_parameters.PH_plot
                                ...
                                XLSX.writetable(output_link*"$(initial_parameters.num_scen)_$(initial_parameters.num_first_stage_var)_$(initial_parameters.num_second_stage_var)_$(initial_parameters.num_const)_$(p)_" * replace(string(Dates.now()), ":" => "_", "." => "_") * "Dual feas cond and primal dual res " * ".xlsx", FW_PH_output)

                                # printing out the info on the identical etries in the feasibility set V
                                io1 = open(output_link*"$(initial_parameters.num_scen)_$(initial_parameters.num_first_stage_var)_$(initial_parameters.num_second_stage_var)_$(initial_parameters.num_const)_$(p)_" * replace(string(Dates.now()), ":" => "_", "." => "_")* " feasibility set V identical entries count" * ".txt", "w")
                                ...
                                io = open(output_link*"$(initial_parameters.num_scen)_$(initial_parameters.num_first_stage_var)_$(initial_parameters.num_second_stage_var)_$(initial_parameters.num_const)_$(p)_" * replace(string(Dates.now()), ":" => "_", "." => "_")* " feasibility set V " * ".txt", "w")
                            ...
                            XLSX.writetable(output_link*"experiments_seed_"*string(initial_parameters.random_seed)*"_date_"*replace(string(Dates.now()), ":" => "_", "." => "_")*".xlsx", output_df)
                        end # while
                        ...
                        for i = 1:length(FW_PH_dual_objective_output)
                            ...
                            savefig(output_link*"$(initial_parameters.num_scen)_$(initial_parameters.num_first_stage_var)_$(initial_parameters.num_second_stage_var)_$(initial_parameters.num_const)_$(p)_" * " FW_PH_dual_value " * replace(string(Dates.now()), ":" => "_", "." => "_") * ".png")
                        end
                    end # for
                end # for
            end # for
        end # for
    elseif experiments_methods[2] + experiments_methods[3]  == 2
        # forming corresponding data frame
        output_df = DataFrame( num_of_scen = Int[], num_fs_var = Int[], num_ss_var = Int[], num_const = Int[], p_RNMDT = Int[], RNMDT_UB = Float64[], RNMDT_x = String[], RNMDT_time = Float64[], RNMDT_wy_gap = Float64[], Solution_method = String[], BnB_UB = Float64[], BnB_LB = Float64[], BnB_x = String[], BnB_time = Float64[], BnB_wy_gap = Float64[], BnB_nodes_explored = Int[], sol_m_iter = Int[] )
            ...
            for i_fs_var in fs_var
                for i_ss_var in ss_var
                    for i_const_num in const_num
                        ...
                        # while we consider all precision factor values
                        while (p_index <= length(p_values))
                            ...
                            if initial_parameters.PH_SDM_parameters.PH_plot
                                ...
                                # solving the RNMDT instance using the BnB+FW-PH method
                                bnb_p_init_time = time()
                                bnb_output, bnb_FW_iterations_output, bnb_p_final_time, bnb_sol_m_iter = bnb_solve(initial_parameters, non_ant_tol, tol_bb, integrality_tolerance)
                                bnb_p_final_time = time() - bnb_p_init_time
                                ...
                                # solving the RNMDT instance using the BnB+FW-PH method
                                bnb_output, bnb_p_final_time, bnb_sol_m_iter = bnb_solve(initial_parameters, non_ant_tol, tol_bb, integrality_tolerance)
                            end
                            ...
                            # global solver or not
                            if RNMDT_global_solved[end]
                                push!(output_df, (initial_parameters.num_scen, initial_parameters.num_first_stage_var, initial_parameters.num_second_stage_var, initial_parameters.num_const, p, objective_value(RNMDT_relaxation), string(value.(RNMDT_relaxation[:x][:,1])), rndmt_final_time, RNMDT_gap_computation( value.(RNMDT_relaxation[:y]), value.(RNMDT_relaxation[:w_RNMDT])), initial_parameters.al_is_used ? "FW-PH" : "BM", bnb_output.UBDg, bnb_output.LBDg, string(bnb_output.soln_val), bnb_p_final_time, bnb_output.RNMDT_gap_wy, bnb_output.nodes_used, bnb_sol_m_iter))
                            else
                                push!(output_df, (initial_parameters.num_scen, initial_parameters.num_first_stage_var, initial_parameters.num_second_stage_var, initial_parameters.num_const, p, 0.0, "NaN", 0.0, 0.0, initial_parameters.al_is_used ? "FW-PH" : "BM", bnb_output.UBDg, bnb_output.LBDg, string(bnb_output.soln_val), bnb_p_final_time,  bnb_output.RNMDT_gap_wy, bnb_output.nodes_used, bnb_sol_m_iter ))
                            end # if
            ...
            for i_fs_var in fs_var
                for i_ss_var in ss_var
                    for i_const_num in const_num
                        ...
                        # defining general intial parameters for the optimisation problem
                        initial_parameters = initialisation(s,i_fs_var,i_ss_var,i_const_num, p, AL, r_seed)
                        @show initial_parameters.quad_mat_dens
                        # if we are using the given pooling problem structure to build up a stochastic instance
                        if initial_parameters.pool_problem_is_used
                                print("THE POOLING PROBLEM IS USED TO CREATE TEST INSTANCES\n")
                                # create a pooling problem 
                        ...
                        # printing the fullscale problem 

                        @show initial_parameters.quad_mat_dens    
                        f = open(output_link*"model_$(r_seed)_$(Int(initial_parameters.quad_mat_dens*100))_$(initial_parameters.num_scen)_$(initial_parameters.num_first_stage_var)_$(initial_parameters.num_second_stage_var)_$(initial_parameters.num_const)_$(p).txt", "w")
                        print(f, primal_problem)
                        close(f)

                        # solving the full scale problem using global solve
                        #optimize!(primal_problem)
                        #primal_problem_f = objective_value(primal_problem)
                        #primal_problem_x = string(value.(primal_problem[:x][:,1]))
                        #primal_problem_optimality_gap = MOI.get(primal_problem, MOI.RelativeGap())
                ...
                for i_ss_var in ss_var
                    for i_const_num in const_num
                        ...
                        while (p_index <= length(p_values))
                            ...
                            an_optimal_solution = value.(RNMDT_relaxation[:y]; result = 1)
                            #optimal_objective = objective_value(RNMDT_relaxation; result = 1)
                            #for ii in 2:result_count(RNMDT_relaxation)
                                #@assert has_values(RNMDT_relaxation; result = ii)
                                #println("Solution $(ii) = ", value.(RNMDT_relaxation[:x]; result = ii))
                                #obj = objective_value(RNMDT_relaxation; result = ii)
                               # println("Objective $(ii) = ", obj)
                                #if isapprox(obj, optimal_objective; atol = 1e-8)
                                   # print("Solution $(ii) is also optimal!")
                                #end
                            #end
                            rndmt_final_time = time() - rndmt_init_time
                            # updating the total time spent by the global solver to solve RNMDT instances
    ...
    elseif experiments_methods[3] == 1
        ...
        # forming corresponding data frame
        output_df = DataFrame( num_of_scen = Int[], num_fs_var = Int[], num_ss_var = Int[], num_const = Int[], p_RNMDT = Int[], Solution_method = String[], BnB_UB = Float64[], BnB_LB = Float64[], BnB_x = String[], BnB_time = Float64[], BnB_wy_gap = Float64[], BnB_nodes_explored = Int[], sol_m_iter = Int[] )
            ...
            for i_fs_var in fs_var
                for i_ss_var in ss_var
                    for i_const_num in const_num
                        ...
                        # while we did not try out all the values in p_values vector
                        while (p_index <= length(p_values))
                            ...
                            if initial_parameters.PH_SDM_parameters.PH_plot
                                ...
                                # Printing the dual_deasibility_condtion and primal_dual_residual resulting from iterations of FW-PH applied to the root node
                                FW_PH_output = DataFrame( iteration = 1:length(bnb_FW_iterations_output[2]), dual_deasibility_cond = dual_feasibility_output, primal_dual_residual = bnb_FW_iterations_output[3])
                                XLSX.writetable(output_link*"$(initial_parameters.num_scen) : scen,$(initial_parameters.num_first_stage_var) : fs_var, $(initial_parameters.num_second_stage_var) : ss_var, $(initial_parameters.num_const) : const, $p : prec_f" * replace(string(Dates.now()), ":" => "_", "." => "_") * "Dual feas cond and primal dual res " * ".xlsx", FW_PH_output)

                                # printing out the info on the identical etries in the feasibility set V
                                io1 = open(output_link*" $(initial_parameters.num_scen) : scen,$(initial_parameters.num_first_stage_var) : fs_var, $(initial_parameters.num_second_stage_var) : ss_var, $(initial_parameters.num_const) : const, $p : prec_f" * replace(string(Dates.now()), ":" => "_", "." => "_")* " feasibility set V identical entries count" * ".txt", "w")
                                for s = 1:length(bnb_FW_iterations_output[5])
                                        println(io1, "\n SCENARIO = $(initial_parameters.num_scen) : $(bnb_FW_iterations_output[5][s])")
                                end
                                close(io1)
                                ...
                                # Printing the fesibility set V resulting from each iteration of FW-PH applied to the root node
                                io = open(output_link*" $(initial_parameters.num_scen) : scen,$(initial_parameters.num_first_stage_var) : fs_var, $(initial_parameters.num_second_stage_var) : ss_var, $(initial_parameters.num_const) : const, $p : prec_f" * replace(string(Dates.now()), ":" => "_", "." => "_")* " feasibility set V " * ".txt", "w")
                                    # printing only for the first scenario to ease representation
                                    for i = 1:length(bnb_FW_iterations_output[4])
                                        println(io, "\n ITERATION = $i : $(length(bnb_FW_iterations_output[4][i][1]))")
                                    end
                            ...
                            # adding the line to the data base
                            push!(output_df, (initial_parameters.num_scen, initial_parameters.num_first_stage_var, initial_parameters.num_second_stage_var, initial_parameters.num_const, p,initial_parameters.al_is_used ? "FW-PH" : "BM", bnb_output.UBDg, bnb_output.LBDg, string(bnb_output.soln_val), bnb_p_final_time, bnb_output.RNMDT_gap_wy, bnb_output.nodes_used, bnb_sol_m_iter))
