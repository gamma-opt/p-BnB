...
"""
function child_node_generation(parent_node::node, var_index::Int, inequality::String, value::Int)
    child_node = node(copy(parent_node.primal_problem), [copy(parent_node.dual_subproblems[j]) for j = 1:length(parent_node.dual_subproblems)])
...
function zero_node_generation(initial_parameters::MIP_initial_parameters)
    ...
    zero_node = node(primal_problem, dual_subproblems)
    return zero_node
end
